...
7. **片段级比较**: 可在归一化时间轴上滑窗比较两条旋律，输出最相似片段位置和距离，减少整曲比较被前奏/间奏/尾奏稀释的问题
8. **维度权重**: UI 可调节时间、音高、力度三个维度的权重比例，距离矩阵和片段比较都会使用加权后的三维曲线
...
归一化后可在 UI 中调节时间、音高、力度权重。权重会直接乘到对应坐标轴上，例如提高音高权重会让距离计算更重视旋律音高轮廓。
